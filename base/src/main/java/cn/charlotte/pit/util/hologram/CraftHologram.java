...
import cn.charlotte.pit.util.hologram.reflection.ClassBuilder;
import cn.charlotte.pit.util.hologram.reflection.NMSClass;
...
abstract class CraftHologram implements Hologram {
    ...
    protected void buildPackets(boolean rebuild) throws Exception {
        ...
        if (this.isTouchable()) {
            ...
            if (HologramAPI.is1_8 && !HologramAPI.useProtocolSupport) {
                touchVehicle = ClassBuilder.buildEntityArmorStand(
                        world,
    ...
    protected void sendNamePackets(final Collection<? extends Player> receivers) {
        for (Player p : receivers) {
            try {
                int id = this.hologramIDs[0];
                Object dataWatcher = this.dataWatcherArmorStand;
