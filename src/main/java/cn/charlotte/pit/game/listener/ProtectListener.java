...
import cn.charlotte.pit.enchantment.type.normal.PurpleGoldEnchant;
import cn.charlotte.pit.enchantment.type.normal.TNTEnchant;
import cn.charlotte.pit.enchantment.type.rare.InstaBoomEnchant;
import cn.charlotte.pit.medal.impl.challenge.ObsidianBreakMedal;
import cn.charlotte.pit.parm.AutoRegister;
import cn.charlotte.pit.perk.type.boost.BuildBattlerBoostPerk;
...
public class ProtectListener implements Listener {
    ...
    private void onPlace(BlockPlaceEvent event) {
        ...
        if (!profile.isEditingMode()) {
            ...
            if ((2 >= location.getBlockX() && location.getBlockX() > -2) && (2 >= location.getBlockZ() && location.getBlockZ() > -2)) {
            ...
            if (event.getBlockPlaced().getType() == Material.TNT) {
                if ("tnt_enchant_item".equals(ItemUtil.getInternalName(event.getItemInHand()))) {
                    ...
                    int enchantLevel = new TNTEnchant().getItemEnchantLevel(event.getPlayer().getInventory().getLeggings());
                ...
                if ("insta_boom_enchant_item".equals(ItemUtil.getInternalName(event.getItemInHand()))) {
                    ...
                    tntPrimed.setMetadata("internal", new FixedMetadataValue(ThePit.getInstance(), "tnt_enchant_item"));
                    tntPrimed.setMetadata("shooter", new FixedMetadataValue(ThePit.getInstance(), event.getPlayer().getUniqueId().toString()));
                    int enchantLevel = new InstaBoomEnchant().getItemEnchantLevel(event.getPlayer().getInventory().getLeggings());
                    tntPrimed.setMetadata("damage", new FixedMetadataValue(ThePit.getInstance(), Math.max(0, enchantLevel)));
            ...
            for (Map.Entry<Integer, PerkData> entry : profile.getChosePerk().entrySet()) {
                if (entry.getValue().getPerkInternalName().equals(new BuildBattlerBoostPerk().getInternalPerkName())) {
    ...
    public void onDamage(EntityDamageByEntityEvent event) {
        ...
        if (event.getDamager() instanceof Player) {
            if (PlayerUtil.isStaffSpectating((Player) event.getDamager())) {
                event.setCancelled(true);
                return;
            }
    ...
    private void onBreak(BlockBreakEvent event) {
        ...
        if (event.getBlock().getType() == Material.OBSIDIAN) {
            ...
            int level = new PurpleGoldEnchant().getItemEnchantLevel(event.getPlayer().getInventory().getLeggings());
            ...
            if (level > 0) {
                ...
                if (level >= 2) {
                    PlayerUtil.addPotionEffect(event.getPlayer(), new PotionEffect(PotionEffectType.REGENERATION, level * 20, 2), true);
                }
            }
        }
