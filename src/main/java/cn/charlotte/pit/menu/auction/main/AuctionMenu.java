package cn.charlotte.pit.menu.auction.main;

import cn.charlotte.pit.ThePit;
import cn.charlotte.pit.events.EventFactory;
import cn.charlotte.pit.events.impl.AuctionEvent;
import cn.charlotte.pit.menu.auction.main.button.AuctionBidButton;
import cn.charlotte.pit.menu.auction.main.button.BidHistoryButton;
...
public class AuctionMenu extends Menu {
    ...
    public static ItemStack convertAuctionIcon(ItemStack itemStack) {
        if (itemStack.clone().getItemMeta().getDisplayName() != null || (itemStack.clone().getItemMeta().getLore() != null && itemStack.clone().getItemMeta().getLore().size() > 0)) {
            ...
            lines.add("");
            lines.add("&e&m*----------------------*");
            if (itemStack.clone().getItemMeta().getDisplayName() != null) {
                lines.add(itemStack.getItemMeta().getDisplayName());
            }
            if (itemStack.clone().getItemMeta().getLore() != null && itemStack.clone().getItemMeta().getLore().size() > 0) {
                lines.addAll(itemStack.clone().getItemMeta().getLore());
            }
            lines.add("&e&m*----------------------*");
            builder.name("&e拍卖物品");
            builder.lore(lines);
